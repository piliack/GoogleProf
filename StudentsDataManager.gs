...
var StudentsDataManagerGP = {
    ...
    getDatas: function (studentsSpreadsheet) {
      ...
      for (var iSheet = 0, lSheets = sheets.length; iSheet < lSheets; iSheet++) {
        ...
        if (sheetName === ConstantsGP.GPSuffixs.STUDENTS_SHEET_STUDENTS_GP) {
          parseStudents(values);
        }
        else if (UtilsGP.testSuffix(sheetName, ConstantsGP.GPSuffixs.STUDENTS_SHEET_DISTRIB)) {
          var distribId = UtilsGP.getFirstPartSuffixed(sheetName, ConstantsGP.GPSuffixs.STUDENTS_SHEET_DISTRIB);
          ...
          for (row = 0; row < lRow; row++) {
            ...
            if (ConstantsGP.StudentsVars[value.toString()] !== undefined) {
              ...
              for (var i = row + 1; i < lRow; i++) {
                var tempValue = colRowArr[col][i];
                tempColValues.push(tempValue ? tempValue : '');
              }
      ...
      function parseDistrib(id, colRowArr) {
        ...
        for (col = 0; col < lCol; col++) {
          ...
          for (row = 0; row < lRow; row++) {
            ...
            for (var i = row + 1; i < lRow; i++) {
              var studentId = colRowArr[col][i];
              if (studentId) {
                groupObj.studentIds.push(studentId);
              }
            }
